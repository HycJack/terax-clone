...
export default function App() {
  ...
  const handleNewSpace = useCallback(async () => {
    // Pop a native directory picker so the user can choose the project
    // folder the new space should open into. Falls back to the current
    // terminal cwd or the home directory if the user cancels.
    let pickedDir: string | null = null;
    try {
      pickedDir = await invoke<string | null>("workspace_pick_directory");
    } catch {
      // picker not available / cancelled — fall through to defaults
    }
    const root = pickedDir || activeCwd || home || null;
    const { spaces, create, setActive } = useSpaces.getState();
    const meta = create({
      name: root ? root.replace(/\\/g, "/").split("/").pop() || `Space ${spaces.length + 1}` : `Space ${spaces.length + 1}`,
      root,
      env: workspaceEnv,
    });
